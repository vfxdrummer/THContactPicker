...
#define kViewPadding 0 // the amount of padding on top and bottom of the view
#define kHorizontalPadding 0 // the amount of padding to the left and right of each contact bubble
#define kVerticalPadding 0 // amount of padding above and below each contact bubble
...
- (void)layoutView {
  ...
  for (id contactKey in self.contactKeys){
    ...
    if ([self.contactKeys count] > 1) {
      if (i == ([self.contactKeys count] - 2)) {
        [contactBubble setPosition:PositionSecondToLast];
      } else if (contactKey == self.contactKeys.lastObject) {
        [contactBubble setPosition:PositionLast];
      } else {
        [contactBubble setPosition:PositionNotLast];
      }
      i++;
    }
